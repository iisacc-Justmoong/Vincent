...
- `LocalCanvasSession`이 최대 16명의 참가자, 크기 제한 프레임, 8초 연결 제한, 프로토콜·세션·UUID 검증, 호스트 권위 단조 revision을 관리한다. 전체 `.vrc` 세션 스냅샷을 350ms 디바운스로 동기화하므로 기본 래스터뿐 아니라 추가 래스터 레이어, 이미지, 텍스트, 도형과 배경 상태도 함께 이동한다.
- 현재 구현은 동일 revision을 기준으로 가장 먼저 호스트가 승인한 전체 스냅샷을 다음 상태로 삼는다. 동시 편집의 구조적 병합은 수행하지 않으며 뒤늦은 제안은 최신 호스트 상태로 재동기화한다.
...
- 프레젠테이션 화면을 좌클릭하면 붉은 레이저 점이 나타난다. 버튼을 누른 채 이동한 경로는 최대 256개 지점으로 제한된 잔상을 남기며, 각 지점은 생성 시점부터 2초에 걸쳐 부드럽게 사라진다.
