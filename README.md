...
- A docked left-side LVRS hierarchy panel lists the raster canvas and current image, shape, text, and raster layers; selecting a raster layer makes it the active paint target, double-clicking a layer name edits it inline, dragging rows changes layer order, and the plus/minus footer buttons create or remove layers
...
- Pan, move, zoom, brush, eraser, shape, fill bucket, and paint-style text tools with a dense full-width top toolbar placed below the empty titlebar drag area, a current-color swatch that opens the HSL color picker, and brush controls that also set text size and text color
