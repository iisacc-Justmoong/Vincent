...
Vincent 6.0 is a private, local-first raster drawing app built with Qt 6 and
...
![Vincent 6.0 running on Windows with a sample raster artwork](docs/marketing/vincent-windows-editor.png)

_Vincent 6.0 on Windows editing a sample raster artwork. The artwork is a
...
## Install (macOS)
1. Download `Vincent-6.0-macOS.pkg` from the latest release.
2. Double-click the installer and follow the prompts to install Vincent 6.0 in Applications.
3. Open Vincent 6.0 from Launchpad or the Applications folder.

## Install (macOS .app bundle)
1. Download `Vincent 6.0.app` from the release assets.
2. Drag it into the Applications folder.
3. Open Vincent 6.0 and allow any prompts to access files you select.

## Install (Linux)
1. Download the `Vincent-6.0-Linux.tar.gz` release archive.
2. Extract it and run `bin/Vincent` from the extracted folder.
3. The archive carries the detected Qt, LVRS, iiPaintEngine, iiSharedCanvas, iiUpdateManager, and iiLicenseManager shared runtimes, QML modules, and Qt platform plugins. An installed tree also provides a `Terminal=false` desktop entry for launching only the GUI on X11 and Wayland.
...
1. Download `Vincent-6.0-Windows.zip` from the release assets.
2. Confirm that the release also provides `Vincent-6.0-Windows.zip.sha256`, verify the archive hash, and check that the extracted `Vincent.exe` has a valid Authenticode signature whose Publisher subject or certificate thumbprint matches the identity stated in the authenticated release notes.
...
The externally signed website release is the single file `Vincent-6.0-Windows.msi`. Its embedded Authenticode signature supplies publisher identity and integrity, so users do not need a separate certificate or checksum file to install it.
...
- Help validate Vincent 6.0 through the [Windows 10/11 testing issue](https://github.com/iisacc-Justmoong/Vincent/issues/18).
