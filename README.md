...
- Pan, move, zoom, brush, eraser, shape, fill bucket, and paint-style text tools with a full-width top toolbar placed directly below the menu bar, 16-pixel horizontal content padding, stock LVRS button geometry, a uniform `Borderless` tone across all toolbar buttons, a current-brush-color circle with a 2-pixel white border that stays visible when the selected color is black and opens the HSL color picker, and brush controls that also set text size and text color
