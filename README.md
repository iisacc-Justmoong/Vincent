...
- Pan, move, zoom, brush, eraser, shape, fill bucket, and paint-style text tools with a dense full-width top toolbar placed below the empty titlebar drag area, toolbar-frame and button x-axis padding set to half the y-axis padding, a current-color swatch that opens the HSL color picker, and brush controls that also set text size and text color
