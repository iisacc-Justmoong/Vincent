...
## Install (macOS)
1. Download `Vincent-4.0.2.pkg` from the latest release.
...
1. Download `Vincent 4.0.2.app` from the release assets.
2. Drag it into the Applications folder.
3. Open Vincent 4.0 and allow any prompts to access files you select.

## Install (Linux)
1. Download the `Vincent-4.0.2-Linux.tar.gz` release archive.
...
1. Download `Vincent-4.0.2-Windows.zip` from the release assets.
2. Confirm that the release also provides `Vincent-4.0.2-Windows.zip.sha256`, verify the archive hash, and check that the extracted `Vincent.exe` has a valid Authenticode signature whose Publisher subject or certificate thumbprint matches the identity stated in the authenticated release notes.
...
The externally signed website release is the single file `Vincent-4.0.2-Windows.msi`. Its embedded Authenticode signature supplies publisher identity and integrity, so users do not need a separate certificate or checksum file to install it.
...
The generic Windows CPack target does not run Qt deployment, PE closure validation, or Authenticode signing. Its deliberately named `Vincent-4.0.2-Windows-unsigned-cpack-incomplete.zip` output is build-system diagnostics only and must never be published.
