...
## Install (macOS)
1. Download `Vincent-4.0.1.pkg` from the latest release.
...
1. Download `Vincent 4.0.1.app` from the release assets.
2. Drag it into the Applications folder.
3. Open Vincent 4.0 and allow any prompts to access files you select.

## Install (Linux)
1. Download the `Vincent-4.0.1-Linux.tar.gz` release archive.
...
1. Download `Vincent-4.0.1-Windows.zip` from the release assets.
2. Confirm that the release also provides `Vincent-4.0.1-Windows.zip.sha256`, verify the archive hash, and check that the extracted `Vincent.exe` has a valid Authenticode signature whose Publisher subject or certificate thumbprint matches the identity stated in the authenticated release notes.
