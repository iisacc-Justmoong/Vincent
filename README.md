...
## Install (Windows)
The preferred future public installation route is the Microsoft Store. A Store-delivered MSIX is certified and re-signed by Microsoft, so Windows users do not need Vincent's private development certificate and do not receive the unknown-publisher or SmartScreen warning associated with a self-signed download. No publicly trusted Vincent 4.0.5 Windows package is currently offered. The ZIP/MSI instructions below apply only after a separately Authenticode-signed non-Store release is published.

1. Download `Vincent-4.0.5-Windows.zip` only if an authenticated release explicitly offers it.
2. Confirm that the release also provides `Vincent-4.0.5-Windows.zip.sha256`, verify the archive hash, and check that the extracted `Vincent.exe` has a valid Authenticode signature whose Publisher subject or certificate thumbprint matches the identity stated in the authenticated release notes.
...
After SignPath approval and signature verification, the externally signed website release will be the single file `Vincent-4.0.5-Windows.msi`. Its embedded Authenticode signature supplies publisher identity and integrity, so users do not need a separate certificate or checksum file to install it.
