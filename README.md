...
- The application window exposes a compact LVRS-styled top menu bar with File, Edit, Window, and Help menus. Its 22-DIP height and background match the dark application window while preserving the existing canvas dialogs, undo/redo, layer actions, tool and shape selection, brush-size changes, canvas view controls, window controls, and complete shortcut reference entries
...
- LVRS solid chrome keeps native window controls. Windows uses its native title bar for dragging, disables the redundant logical drag handle, and leaves no reserved gap above the toolbar; other desktop platforms retain the LVRS drag surface when needed
