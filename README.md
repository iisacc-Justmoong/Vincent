...
Vincent supports Linux source builds, but no public Linux binary archive is currently offered. Configure and build the tagged source with the required Qt, LVRS, iiPaintEngine, iiSharedCanvas, and iiUpdateManager dependencies; the packaging target collects the Qt, LVRS, iiPaintEngine, iiSharedCanvas, and iiUpdateManager shared runtimes, and an installed tree provides `bin/Vincent` plus a `Terminal=false` desktop entry for launching only the GUI on X11 and Wayland.
