...
launch-verification request; documents remain local and Vincent contains no
...
per-launch identifier, and online/offline state; they contain no profile,
device, account, or document data and never target an Internet service.
...
- Privacy: License enforcement is temporarily disabled, so startup performs no credential read or license-validation request. Opening Preferences explicitly asks `iiLicenseManager 0.2` for the account email associated with its verified offline Vincent activation. If that activation has not yet been created, a one-time compatibility path reads the existing secure credential only inside C++ and submits it to iiLicenseManager's server-authoritative activation endpoint; the key never enters QML. Documents and editing activity stay local. Vincent contains no telemetry, analytics, advertising, remote polling, or automatic update checks. When the user-controlled nearby-discovery setting is enabled, a one-hop local multicast heartbeat advertises only an ephemeral Vincent session identifier and presence state; it sends no profile, device, account, or document data. A manual **Check for Updates…** makes one version-manifest request; **Update now** alone may read an existing secure credential to authorize and download a verified installer.
...
- Pan, move, zoom, brush, eraser, shape, fill bucket, and paint-style text tools with a full-width top toolbar placed directly below the menu bar, 16-pixel horizontal content padding, stock LVRS button geometry, a uniform `Borderless` tone across all toolbar buttons, a left-aligned current-brush-color circle that opens the HSL color picker, and a matching border-free trailing circular profile placeholder with no action yet; brush controls also set text size and text color
