...
void tst_WindowsBuildWorkflowContract::storePackagedBuildDisablesExternalUpdater()
{
    ...
    QVERIFY(manager.contains(QStringLiteral("return packageResult == APPMODEL_ERROR_NO_PACKAGE;")));
    ...
    QVERIFY(qml.contains(QStringLiteral("enabled: VincentUpdateManager.selfUpdateSupported")));
    QVERIFY(qml.contains(QStringLiteral("visible: VincentUpdateManager.selfUpdateSupported")));
    ...
    QVERIFY(buildGuide.contains(QStringLiteral("Windows Store/MSIX packaged context is detected at "
                                               "runtime through `GetCurrentPackageFullName`")));
}
...
void tst_WindowsBuildWorkflowContract::correspondingSourceToolDefinesImmutableReleaseContract()
{
    ...
    QVERIFY(source.contains(
        QStringLiteral("throw \"Vincent release revision does not declare version $Version.\"")));
    ...
    QVERIFY(source.contains(QStringLiteral("2ceeb8c0cf81d01e9740df1635badc08d5a12ebe")));
    QVERIFY(source.contains(QStringLiteral("IiSharedCanvasRevision")));
    QVERIFY(source.contains(QStringLiteral("IiSharedCanvasSource")));
    QVERIFY(source.contains(QStringLiteral("-Label \"iiSharedCanvas\"")));
    QVERIFY(source.contains(QStringLiteral("/iiSharedCanvas/CMakeLists.txt")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("$buildGuide = $buildGuideTemplate.Replace(\"@@VERSION@@\", $Version)")));
    QVERIFY(source.contains(QStringLiteral("From `Vincent`, configure, build, and test using the "
                                           "repository-local `build/` directory.")));
...
void tst_WindowsBuildWorkflowContract::windowsBuildScriptDefinesRunnablePackageContract()
{
    ...
    QVERIFY(source.contains(
        QStringLiteral("$SignedMsiPath = Join-Path $BuildDir \"Vincent-$Version-Windows.msi\"")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Invoke-Native $heat @(\"dir\", $SourceDirectory, \"-wx\"")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("\"--qmldir\", (Join-Path $RepositoryRoot \"App\\qml\")")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("Remove-EmbeddedDependencyQmlImport -ModuleName \"LVRS\"")));
    QVERIFY(source.contains(QStringLiteral(
    ...
    QVERIFY(
        source.contains(QStringLiteral("[System.Diagnostics.FileVersionInfo]::GetVersionInfo")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("\"--skip-plugin-types\", \"qmltooling,generic,sqldrivers\"")));
    QVERIFY(source.contains(QStringLiteral("\"--no-system-d3d-compiler\"")));
    QVERIFY(source.contains(QStringLiteral("\"--no-opengl-sw\"")));
    QVERIFY(
        source.contains(QStringLiteral("\"--exclude-plugins\", \"qpdf,qtvirtualkeyboardplugin\"")));
    QVERIFY(source.contains(QStringLiteral("\"--verbose\", \"0\"")));
    QVERIFY(source.contains(
        QStringLiteral("Verify-WindowsStage -Directory $StageDir -ResolvedQtPrefix $QtPrefix "
                       "-ExpectedFileVersion $WindowsFileVersion -BuildType $BuildType")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Public Windows packaging requires an explicit iiPaintEngine LICENSE")));
    QVERIFY(source.contains(QStringLiteral("qtimageformats")));
    ...
    const QString pretendardLicensePath =
        QFINDTESTDATA("../packaging/windows/licenses/Pretendard-OFL-1.1.txt");
    QVERIFY2(!pretendardLicensePath.isEmpty(), "Pretendard OFL license was not found");
    QVERIFY(readTextFile(pretendardLicensePath)
                .contains(QStringLiteral("SIL OPEN FONT LICENSE Version 1.1")));

    const QString psdLicensePath =
        QFINDTESTDATA("../packaging/windows/licenses/psd_sdk-BSD-2-Clause.txt");
    ...
    const QString minizLicensePath =
        QFINDTESTDATA("../packaging/windows/licenses/psd_sdk-miniz-Unlicense.txt");
    QVERIFY2(!minizLicensePath.isEmpty(), "psd_sdk miniz Unlicense was not found");
    QVERIFY(
        readTextFile(minizLicensePath).contains(QStringLiteral("free and unencumbered software")));
    const QString qtKeychainLicensePath =
        QFINDTESTDATA("../packaging/common/licenses/QtKeychain-BSD-3-Clause.txt");
    QVERIFY2(!qtKeychainLicensePath.isEmpty(), "QtKeychain BSD license was not found");
    QVERIFY(readTextFile(qtKeychainLicensePath)
                .contains(QStringLiteral("Redistribution and use in source and binary forms")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Rebuild those dependencies with the same MinGW kit as Qt")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Remove-Item -LiteralPath $TargetDirectory -Recurse -Force")));
}
...
void tst_WindowsBuildWorkflowContract::windowsMsiDefinitionProvidesInstallOptions()
{
    ...
    QVERIFY(source.contains(
        QStringLiteral("<Property Id=\"ApplicationFolderName\" Value=\"Vincent\" />")));
    QVERIFY(source.contains(
        QStringLiteral("<Property Id=\"WixAppFolder\" Value=\"WixPerUserFolder\" />")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("$productSource.Replace(\"%%PRODUCT_CODE%%\", $productCode)")));
    QVERIFY(!source.contains(QStringLiteral("<Product Id=\"*\"")));
    QVERIFY(source.contains(
        QStringLiteral("<Property Id=\"VINCENT_EXISTING_USER_CONTEXT\" Secure=\"yes\">")));
    QVERIFY(source.contains(
        QStringLiteral("<Property Id=\"VINCENT_EXISTING_MACHINE_CONTEXT\" Secure=\"yes\">")));
    QVERIFY(source.contains(
        QStringLiteral("<Property Id=\"VINCENT_MACHINE_PROGRAMFILES64\" Secure=\"yes\">")));
    QVERIFY(source.contains(QStringLiteral("Name=\"ProgramFilesDir\"")));
    QVERIFY(source.contains(
        QStringLiteral("<Property Id=\"VINCENT_EXISTING_USER_INSTALLLOCATION\" Secure=\"yes\">")));
    QVERIFY(source.contains(QStringLiteral(
    ...
    QVERIFY(source.contains(QStringLiteral("<Component Id=\"InstallContextComponent\" "
                                           "Guid=\"3048C76F-C0FC-4CEE-9C86-D154BDA6BCD8\"")));
    ...
    const QRegularExpression existingMachineContextAction(QStringLiteral(
        R"(<CustomAction\s+Id="VincentSetExistingMachineContext"\s+Property="ALLUSERS"\s+Value="1"\s+Execute="firstSequence"\s*/>)"));
    ...
    QVERIFY(source.contains(
        QStringLiteral("WIX_UPGRADE_DETECTED AND NOT (VINCENT_EXISTING_MACHINE_CONTEXT OR "
                       "VINCENT_LEGACY_MACHINE_CONTEXT)")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("<Property Id=\"DISABLEADVTSHORTCUTS\" Value=\"1\" />")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Add-AdvertisedStartMenuShortcut -HarvestPath $runtimePath")));
...
void tst_WindowsBuildWorkflowContract::windowsBuildScriptDefinesAuthenticodeContract()
{
    ...
    QVERIFY(source.contains(
        QStringLiteral("temporary unsigned public release does not allow -SkipTests")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Public release signing rejects self-signed certificate chains")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("-Sign and -AllowUnsignedPackage cannot be used together")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("Authenticode release signing does not allow -SkipTests")));
    QVERIFY(
        source.contains(QStringLiteral("\"Vincent.exe\", \"LVRS.dll\", \"libiiPaintEngine.dll\"")));
    QVERIFY(
        source.contains(QStringLiteral("-ExpectedCertificateThumbprint $CertificateThumbprint")));
    QVERIFY(!source.contains(QStringLiteral("Get-Command \"signtool.exe\"")));

    const qsizetype stripIndex =
        source.lastIndexOf(QStringLiteral("Strip-WindowsRuntimeBinaries -Directory"));
    const qsizetype structuralVerifyIndex =
        source.lastIndexOf(QStringLiteral("Verify-WindowsStage -Directory"));
    const qsizetype signStageIndex =
        source.lastIndexOf(QStringLiteral("Sign-WindowsStage -Directory"));
    const qsizetype signatureVerifyIndex =
        source.lastIndexOf(QStringLiteral("Verify-WindowsStageSignatures -Directory"));
    const qsizetype zipIndex = source.lastIndexOf(QStringLiteral("Compress-Archive"));
    const qsizetype zipChecksumIndex =
        source.lastIndexOf(QStringLiteral("-File $ZipPartialPath `"));
    const qsizetype groupPublishIndex =
        source.lastIndexOf(QStringLiteral("-Artifacts $packageArtifacts `"));
    ...
    const qsizetype createMsiIndex = source.lastIndexOf(
        QStringLiteral("New-MsiInstaller -SourceDirectory $StageDir -OutputPath $MsiPartialPath"));
    const qsizetype signMsiIndex =
        source.lastIndexOf(QStringLiteral("Sign-AuthenticodeFile -File $MsiPartialPath"));
    const qsizetype verifyMsiIndex =
        source.lastIndexOf(QStringLiteral("Verify-AuthenticodeFile -File $MsiPartialPath"));
    const qsizetype databaseContractIndex =
        source.lastIndexOf(QStringLiteral("Assert-MsiDatabaseContract -MsiFile $MsiPartialPath"));
    const qsizetype msiChecksumIndex =
        source.lastIndexOf(QStringLiteral("-File $MsiPartialPath `"));
    ...
    const qsizetype signingIdentityIndex = source.lastIndexOf(
        QStringLiteral("$ResolvedSignTool = Resolve-SignTool -ConfiguredPath $SignToolPath"));
    const qsizetype recoveryIndex =
        source.lastIndexOf(QStringLiteral("-Artifacts $recoveryArtifacts `"));
    const qsizetype clearArtifactsIndex = source.lastIndexOf(
        QStringLiteral("Clear-WindowsPackageArtifacts -Paths $partialArtifacts"));
    const qsizetype toolchainIndex =
        source.lastIndexOf(QStringLiteral("Write-Step \"Resolving toolchain\""));
    ...
    QVERIFY(source.contains(
        QStringLiteral("$partialArtifacts += @($ZipPartialPath, $ZipChecksumPartialPath)")));
    QVERIFY(source.contains(QStringLiteral(
    ...
    QVERIFY(source.contains(
        QStringLiteral("Another build-windows.ps1 process is already using this repository")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Test-Path -LiteralPath $PackagePublicationJournalPath -PathType Leaf")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("JournalPath = $UnsignedPackagePublicationJournalPath")));
    ...
    QVERIFY(!source.contains(
        QStringLiteral("Remove-Item -LiteralPath $ZipPath -Force -ErrorAction SilentlyContinue")));
    QVERIFY(!source.contains(
        QStringLiteral("Remove-Item -LiteralPath $MsiPath -Force -ErrorAction SilentlyContinue")));
}
...
void tst_WindowsBuildWorkflowContract::cmakeBuildsWindowsGuiExecutable()
{
    ...
    QVERIFY2(source.contains(QStringLiteral("qt_add_executable(Vincent\n"
                                            ...
                                            "        WIN32\n"
                                            "        MACOSX_BUNDLE")),
             "Vincent must use the Windows GUI subsystem so launching it does not create a console "
             "window");
}
...
void tst_WindowsBuildWorkflowContract::windowsResourcesDefineNativeApplicationContract()
{
    ...
    QVERIFY(resourceTemplate.contains(
        QStringLiteral("CREATEPROCESS_MANIFEST_RESOURCE_ID RT_MANIFEST")));
    QVERIFY(resourceTemplate.contains(QStringLiteral("VS_VERSION_INFO VERSIONINFO")));
    QVERIFY(
        resourceTemplate.contains(QStringLiteral("FILEVERSION @VINCENT_WINDOWS_VERSION_COMMA@")));
    QVERIFY(resourceTemplate.contains(
        QStringLiteral("VALUE \"FileDescription\", \"Vincent raster drawing application\\0\"")));
...
void tst_WindowsBuildWorkflowContract::cmakeHasWindowsInstallAndPackageRules()
{
    ...
    QVERIFY(source.contains(QStringLiteral("elseif(WIN32)\n    install(TARGETS Vincent RUNTIME "
                                           "DESTINATION \".\" COMPONENT Runtime)")));
    QVERIFY(source.contains(QStringLiteral("elseif(WIN32)\n    set(CPACK_GENERATOR \"ZIP\")")));
    QVERIFY(source.contains(
        QStringLiteral("set(CPACK_PACKAGE_FILE_NAME "
                       "\"Vincent-${CPACK_PACKAGE_VERSION}-Windows-unsigned-cpack-incomplete\")")));
    QVERIFY(source.contains(
        QStringLiteral("if(WIN32)\n    add_custom_command(TARGET Vincent POST_BUILD")));
    QVERIFY(source.contains(QStringLiteral("if(WIN32 AND MINGW)")));
    QVERIFY(source.contains(QStringLiteral(
        "get_filename_component(_vincent_mingw_runtime_dir \"${CMAKE_CXX_COMPILER}\" DIRECTORY)")));
    ...
    QVERIFY(source.contains(QStringLiteral("set_source_files_properties(\"${psd_sdk_SOURCE_DIR}/"
                                           "src/Psd/Psdminiz.c\" PROPERTIES LANGUAGE CXX)")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("lvrs_apply_platform_build_optimizations(vincent_psd_sdk)")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("configure_file(\"${VINCENT_WINDOWS_RESOURCE_TEMPLATE}\"")));
    ...
    QVERIFY(testsSource.contains(
        QStringLiteral("PATH=path_list_prepend:$<TARGET_FILE_DIR:iiPaintEngine::iiPaintEngine>")));
    ...
    QVERIFY(databaseContractSource.contains(
        QStringLiteral("[Parameter(Mandatory = $true)]\n    [string]$MsiPath")));
    QVERIFY(!databaseContractSource.contains(QStringLiteral("Vincent-$Version-Windows.msi")));
    QVERIFY(
        !databaseContractSource.contains(QStringLiteral("Vincent-$Version-Windows-unsigned.msi")));
    ...
    QVERIFY(buildScriptSource.contains(QStringLiteral("function Assert-MsiDatabaseContract {\n"
                                                      "    param(\n"
                                                      "        [string]$MsiFile,\n"
                                                      "        [string]$ProductVersion\n"
                                                      "    )")));
    QVERIFY(buildScriptSource.contains(QStringLiteral("\"-Version\", $ProductVersion")));
...
void tst_WindowsBuildWorkflowContract::appEntryPointAvoidsNonExportedLvrsRuntimeSymbols()
{
    ...
    QVERIFY(source.contains(
        QStringLiteral("QCoreApplication::applicationDirPath() + QStringLiteral(\"/qml\")")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("engine.singletonInstance<QObject *>(QStringLiteral(\"LVRS\"),")));
...
void tst_WindowsBuildWorkflowContract::appEntryPointShowsFinalGeometryOnlyOnce()
{
    ...
    QVERIFY(
        source.contains(QStringLiteral("void showLaunchWindow(QQmlApplicationEngine &engine)")));
    QVERIFY(source.contains(QStringLiteral("QSize finalLaunchWindowSize(const QWindow &window)")));
    QVERIFY(source.contains(QStringLiteral("constexpr int initialLaunchWindowWidth = 1280;")));
    QVERIFY(source.contains(
        QStringLiteral("QSize initialLaunchWindowSize(const QSize &frameworkSize)")));
    QVERIFY(source.contains(
        QStringLiteral("qRound(qreal(initialLaunchWindowWidth) * frameworkSize.height() / "
                       "frameworkSize.width())")));
    QVERIFY(source.contains(
        QStringLiteral("const QSize requestedSize = initialLaunchWindowSize(window.size());")));
    QVERIFY(source.contains(QStringLiteral("screen->availableGeometry().size()")));
    QVERIFY(source.contains(
        QStringLiteral("requestedSize.scaled(availableSize, Qt::KeepAspectRatio)")));
    ...
    const qsizetype loadIndex = source.indexOf(QStringLiteral(
        "engine.loadFromModule(QStringLiteral(\"Vincent\"), QStringLiteral(\"Main\"));"));
...
void tst_WindowsBuildWorkflowContract::buildGuideDocumentsWindowsScript()
{
    ...
    QVERIFY(source.contains(
        QStringLiteral("## 1b. Windows Build, Package, and Current-User Install Script")));
    QVERIFY(source.contains(QStringLiteral(
        "powershell -ExecutionPolicy Bypass -File .\\build-windows.ps1 -Clean -SkipPackage")));
    QVERIFY(source.contains(
        QStringLiteral("powershell -ExecutionPolicy Bypass -File .\\build-windows.ps1 -SkipPackage "
    ...
    QVERIFY(source.contains(
        QStringLiteral("preserves that repository-local tree for incremental work")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("Unattended upgrades must not override `ALLUSERS` or `MSIINSTALLPERUSER`")));
    ...
    QVERIFY(source.contains(QStringLiteral("LVRS stock application-window geometry")));
    QVERIFY(source.contains(QStringLiteral("1,280 logical pixels")));
    QVERIFY(source.contains(QStringLiteral("aspect ratio")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("checksum alone proves equality, not publisher identity")));
    ...
    QVERIFY(
        source.contains(QStringLiteral("expected Publisher subject or certificate thumbprint")));
    ...
    QVERIFY(source.contains(
        QStringLiteral("build-windows-store.ps1 -Mode Development -InstallDevelopment")));
    ...
    QVERIFY(source.contains(QStringLiteral("privateNetworkClientServer")));
    QVERIFY(source.contains(QStringLiteral("Microsoft re-signs")));

    const QString storeManifestPath = QFINDTESTDATA("../packaging/windows/AppxManifest.xml.in");
    QVERIFY2(!storeManifestPath.isEmpty(), "Windows Store manifest template was not found");
    const QString storeManifest = readTextFile(storeManifestPath);
    QVERIFY(storeManifest.contains(
        QStringLiteral("<Capability Name=\"privateNetworkClientServer\" />")));

    const QString storeScriptPath = QFINDTESTDATA("../build-windows-store.ps1");
    QVERIFY2(!storeScriptPath.isEmpty(), "build-windows-store.ps1 test data was not found");
    const QString storeScript = readTextFile(storeScriptPath);
    QVERIFY(storeScript.contains(QStringLiteral("$Version = \"5.1\"")));
    QVERIFY(storeScript.contains(QStringLiteral("while ($packageVersionParts.Count -lt 4)")));
    QVERIFY(storeScript.contains(
        QStringLiteral("$PackageVersion = $packageVersionParts[0..3] -join \".\"")));
}
...
void tst_WindowsBuildWorkflowContract::signPathWorkflowDefinesFreeWebsiteReleaseContract()
{
    ...
    QVERIFY(workflow.contains(QStringLiteral("iisacc-Justmoong/iiSharedCanvas")));
    QVERIFY(
        workflow.contains(QStringLiteral("LVRS_COMMIT: 8132a48e4c0788f0548bff20fd7089878a09c760")));
    QVERIFY(workflow.contains(
        QStringLiteral("IIPAINTENGINE_COMMIT: 9979eae5f7925576ec2fb9f780ddbb0fcecf64dc")));
    QVERIFY(workflow.contains(
        QStringLiteral("IISHAREDCANVAS_COMMIT: 2ceeb8c0cf81d01e9740df1635badc08d5a12ebe")));
    ...
    QVERIFY(workflow.contains(QStringLiteral("IIUPDATEMANAGER_READ_TOKEN")));
    ...
    const qsizetype lvrsBuildIndex =
        workflow.indexOf(QStringLiteral("Build and install pinned LVRS"));
    ...
    QVERIFY(workflow.contains(QStringLiteral("Join-Path $env:IIPAINTENGINE_PREFIX \"bin\"")));
    QVERIFY(workflow.contains(
        QStringLiteral("Join-Path $env:IIPAINTENGINE_PREFIX \"platforms\\windows\\bin\"")));
    QVERIFY(workflow.contains(
        QStringLiteral("Join-Path $env:LVRS_PREFIX \"platforms\\windows\\bin\"")));
    QVERIFY(workflow.contains(
        QStringLiteral("$env:PATH = (@($testRuntimePaths) + @($env:PATH)) -join \";\"")));
    ...
    QVERIFY(workflow.contains(
        QStringLiteral("-DLVRS_BOOTSTRAP_LVRS_ENABLE_PLATFORM_BUILD_OPTIMIZATIONS=OFF")));
    ...
    QVERIFY(workflow.contains(
        QStringLiteral("powershell -NoProfile -ExecutionPolicy Bypass -File "
                       ".\\tests\\tst_windowsmsidatabasecontract.ps1 "
                       "-BuildDirectory .\\build -Version $msiVersion -MsiPath $msi")));
    QVERIFY(workflow.contains(
        QStringLiteral("$msiVersion = if ($expectedVersion -match '^\\d+\\.\\d+$') { "
                       "\"$expectedVersion.0\" } else { $expectedVersion }")));
    QVERIFY(workflow.contains(
        QStringLiteral("throw \"MSI database contract rejected the signed MSI.\"")));
    QVERIFY(workflow.contains(
        QStringLiteral("Extract signed MSI payload for nested signature verification")));
    QVERIFY(workflow.contains(QStringLiteral("Vincent.exe")));
    QVERIFY(workflow.contains(
        QStringLiteral("The installed Vincent executable is not Authenticode-signed")));
    QVERIFY(
        workflow.contains(QStringLiteral("SignTool rejected the installed Vincent executable")));
    ...
    QVERIFY2(!sourceWorkflowPath.isEmpty(),
             "The Windows corresponding-source workflow was not found");
    ...
    QVERIFY(sourceWorkflow.contains(QStringLiteral("Vincent-5.1-Corresponding-Source.zip")));
    QVERIFY(sourceWorkflow.contains(
        QStringLiteral("IISHAREDCANVAS_COMMIT: 2ceeb8c0cf81d01e9740df1635badc08d5a12ebe")));
    QVERIFY(sourceWorkflow.contains(QStringLiteral("iisacc-Justmoong/iiSharedCanvas")));
    QVERIFY(sourceWorkflow.contains(
        QStringLiteral("-IiSharedCanvasRevision $env:IISHAREDCANVAS_COMMIT")));
    QVERIFY(sourceWorkflow.contains(QStringLiteral("Require pinned iiUpdateManager provenance")));
    QVERIFY(sourceWorkflow.contains(QStringLiteral("IIUPDATEMANAGER_READ_TOKEN")));
    QVERIFY(
        sourceWorkflow.contains(QStringLiteral("repository: iisacc-Justmoong/iiUpdateManager")));
    QVERIFY(sourceWorkflow.contains(QStringLiteral("persist-credentials: false")));
    ...
    QVERIFY(readme.contains(
        QStringLiteral("No SignPath Foundation certificate is currently active for Vincent")));
    QVERIFY(readme.contains(
        QStringLiteral("publishes an explicitly named unsigned website artifact through 2026")));
    ...
    QVERIFY(readme.contains(QStringLiteral("License enforcement is temporarily disabled")));
    QVERIFY(readme.contains(
        QStringLiteral("startup performs no credential read or license-validation request")));
    QVERIFY(readme.contains(
        QStringLiteral("Update now** alone may read an existing secure credential")));
    ...
    QVERIFY(
        readme.contains(QStringLiteral("https://github.com/iisacc-Justmoong/Vincent/discussions")));
    QVERIFY(
        readme.contains(QStringLiteral("https://github.com/iisacc-Justmoong/Vincent/issues/18")));
    ...
    QVERIFY(
        contributing.contains(QStringLiteral("QML changes must use the `.local/LVRS/` framework")));
    QVERIFY(contributing.contains(
        QStringLiteral("self-signed SignPath trial outputs are development-only artifacts")));
