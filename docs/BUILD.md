# Vincent 5.1 Packaging Guide

This document captures the end-to-end steps needed to turn the `Vincent` build tree into an App Store–compliant macOS package for Vincent 5.1, a Windows runtime package, plus the Linux TGZ package. It assumes you have the Qt dependencies referenced in the repository README installed.
...
After `macdeployqt`, the script removes absolute build-machine `LC_RPATH` entries from every bundled Mach-O with `install_name_tool`, requires the linked `libiiUpdateManager*.dylib` under `Contents/Frameworks`, then inspects every Mach-O file with `otool -L` and every remaining `LC_RPATH` with `otool -l`. Packaging fails if the updater runtime is absent, has the wrong architecture/deployment target, or a deployed binary still references an absolute non-system dependency or RPATH. The `.pkg` payload is inspected for the updater runtime before publication.
...
Run the Windows build from Windows PowerShell 5.1 or newer. The script expects Windows-built Qt, LVRS, iiPaintEngine, iiSharedCanvas, and iiUpdateManager prefixes; macOS `.local` binaries cannot be reused on Windows. Set the prefix variables once per shell session:
...
6. Copy the actual 40-hex thumbprint without inventing or shortening it. Set only non-secret selectors and the CA's RFC 3161 URL, then close and reopen PowerShell so the persistent user environment is reloaded:
   ...
   [Environment]::SetEnvironmentVariable(
     ...
     "https://<publisher-controlled-location>/Vincent-5.1-Corresponding-Source.zip",
     "User"
   )
   [Environment]::SetEnvironmentVariable(
...
Do not publish the generic Windows CPack output. It does not run `windeployqt`, staged PE closure checks, or Authenticode signing and is deliberately named `Vincent-5.1-Windows-unsigned-cpack-incomplete.zip`.
...
```powershell
$zip = (Resolve-Path .\dist\Vincent-5.1-Windows.zip).Path
...
$auditDir = Join-Path $env:TEMP ("Vincent-5.1-release-audit-" + [Guid]::NewGuid().ToString("N"))
...
- `dist/Vincent-5.1-Windows-Store-x64.msix`, intentionally unsigned for Store ingestion;
- `dist/Vincent-5.1-Windows-Store-x64.msixupload`, a ZIP container holding exactly that x64 MSIX and no fake symbol archive;
