...
The workflow requests only Qt 6.8.3 modules that the online installer exposes separately; Qt SVG remains part of the base desktop package. LVRS and iiPaintEngine are installed in separate observable steps. The pinned iiPaintEngine installer configures single-configuration generators as Release so its exported target contains a usable `IMPORTED_IMPLIB_RELEASE` for the Vincent Release configure. The GitHub-hosted runner builds the pinned LVRS dependency as `MinSizeRel` and disables both its general IPO switch and its platform build optimizations, because the latter independently enables per-configuration LTO and can trigger a MinGW 13 internal compiler error while linking the generated QML resources. The pinned LVRS revision makes its framework bootstrap action forward both controls and contains the cache-argument fix that preserves explicit `OFF` values through the nested platform configure; older revisions either ignored the controls or silently omitted their false values and re-enabled LTO. `MinSizeRel` remains an official optimized distribution configuration. Vincent itself is still built as Release and runs the complete functional and packaging validation. Qt GUI tests use the offscreen platform on non-interactive runners, and the Windows PowerShell policy test imports its built-in Utility module from `$PSHOME` so an inherited PowerShell 7 module path cannot hide `Get-FileHash`. The PowerShell step passes the Rust CLI's `--` CMake-argument separator through an explicit argument array, preventing the shell from consuming it, and places the LVRS example/test exclusions before that separator so they remain CLI options. The workflow discovers exactly one versioned `Vincent-*-Windows.msi` at both the SignPath input and output boundaries, so a release version increase cannot silently leave a stale hard-coded workflow path. It verifies the returned Authenticode chain, SignPath Foundation subject, timestamp, and MSI database contract. It then extracts an administrative image and independently verifies the nested `Vincent.exe` signature, signer, and RFC 3161 timestamp so a misconfigured artifact policy cannot publish an outer-only signature. The signed-output verification reads `CMAKE_PROJECT_VERSION:STATIC` from the generated cache and passes the build directory and configured project version to the MSI database contract, so every mandatory contract input is explicit and version drift fails closed. A signing request also fails before submission unless its immutable Git tag exactly matches that configured project version and all SignPath identifiers are present. After SignPath approves and returns the artifact, download the unarchived workflow artifact `Vincent-website-release`, install/launch/remove it on a clean Windows system, and place only the returned `Vincent-4.0.5-Windows.msi` under `dist/` and on the website. The standalone MSI is sufficient for installation; an adjacent checksum can be retained internally but is not required because Authenticode binds the complete installer and publisher identity.
