...
The script always configures with `cmake -S . -B build`, preserves that repository-local tree for incremental work, builds with CMake's parallel mode, and runs `ctest --test-dir build --output-on-failure` unless `-SkipTests` is passed. Use `powershell -ExecutionPolicy Bypass -File .\build-windows.ps1 -Clean` only after changing toolchains or when recovering a stale build tree. Passing `-SkipTests` configures `BUILD_TESTING=OFF` and builds only the `Vincent` target. Runtime deployment uses `windeployqt --qmldir App/qml --translations en,ko`, omits the release-only QML debugger plugin group plus the unused PDF, Qt Virtual Keyboard, PostgreSQL, and Mimer SQL plugins, then copies the LVRS and iiPaintEngine runtime DLLs. LVRS QML is compiled into the LVRS binary, so the stage deliberately removes any loose `qml/LVRS` directory emitted by deployment instead of copying thousands of duplicate source files or rewriting `qmldir` `prefer` directives.
