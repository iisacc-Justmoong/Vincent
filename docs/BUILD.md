# Vincent 2.1.0 Packaging Guide

This document captures the end-to-end steps needed to turn the `Vincent` build tree into an App Store–compliant macOS package for Vincent 2.1.0, plus the Linux TGZ package. It assumes you have the Qt dependencies referenced in the repository README installed.
...
The built app lives under `build-release/"Vincent 2.1.0.app"`. Copy it to a staging directory (for example, `dist/"Vincent 2.1.0.app"`) so you can safely run deployment tools without touching your build tree.

## 4. Embed Qt Frameworks
Run `macdeployqt` in App Store mode to embed the required Qt frameworks and QML plugins:
```bash
macdeployqt "dist/Vincent 2.1.0.app" \
  -appstore-compliant \
  -qmldir=App/qml \
  -always-overwrite
```
Verify that all `.framework` bundles now sit inside `dist/"Vincent 2.1.0.app"/Contents/Frameworks` and that `qt.conf` exists in `Contents/Resources/`.

## 5. Prepare Metadata
Update the generated `Info.plist` (inside `dist/"Vincent 2.1.0.app"/Contents/`) with:
...
- Any usage description strings your app requires (e.g., `NSMicrophoneUsageDescription`)—Vincent 2.1.0 currently relies only on file picker access.
...
codesign --force --options runtime \
  ...
  "dist/Vincent 2.1.0.app"
```
Then validate the signature:
```bash
codesign --verify --deep --strict "dist/Vincent 2.1.0.app"
spctl --assess --type execute "dist/Vincent 2.1.0.app"
```
...
- Use `otool -L "dist/Vincent 2.1.0.app"/Contents/MacOS/"Vincent 2.1.0"` to ensure no absolute paths to your build tree remain.
