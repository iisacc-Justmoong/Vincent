...
6. Copy the actual 40-hex thumbprint without inventing or shortening it. Set only non-secret selectors and the CA's RFC 3161 URL, then close and reopen PowerShell so the persistent user environment is reloaded:
   ...
   [Environment]::SetEnvironmentVariable(
     ...
     "https://<publisher-controlled-location>/Vincent-4.0.4-Corresponding-Source.zip",
     "User"
   )
   [Environment]::SetEnvironmentVariable(
...
Do not publish the generic Windows CPack output. It does not run `windeployqt`, staged PE closure checks, or Authenticode signing and is deliberately named `Vincent-4.0.4-Windows-unsigned-cpack-incomplete.zip`.
...
```powershell
$zip = (Resolve-Path .\dist\Vincent-4.0.4-Windows.zip).Path
...
$auditDir = Join-Path $env:TEMP ("Vincent-4.0.4-release-audit-" + [Guid]::NewGuid().ToString("N"))
...
`build-windows.ps1 -ExternalSigning -SkipPackage -CreateMsi` is deliberately separate from `-AllowUnsignedPackage`. The external-signing mode accepts only Release or MinSizeRel, requires the complete test suite and public corresponding-source evidence, creates exactly `build/signpath-input/Vincent-4.0.4-Windows.msi`, and labels the staged files and MSI as non-distributable input. It does not publish anything under `dist/`. `-ExternalSigning` cannot be combined with `-Sign` or `-AllowUnsignedPackage`, cannot create a ZIP, and cannot skip MSI creation.
...
- `dist/Vincent-4.0.4-Windows-Store-x64.msix`, intentionally unsigned for Store ingestion;
- `dist/Vincent-4.0.4-Windows-Store-x64.msixupload`, a ZIP container holding exactly that x64 MSIX and no fake symbol archive;
...
1. Open Transporter.
2. Drag `dist/Vincent-4.0.4.pkg` into the queue.
