...
- Presents a move tool for inserted image, text, and shape layer objects; clicking an object selects it, dragging the body moves it, dragging enlarged corner or edge handles resizes its bounds with matching resize cursors, transforms may extend outside the canvas like Photoshop layers, object pixels and transform chrome remain clipped to the canvas visual scope, and Delete or Backspace removes the selected object before export.
